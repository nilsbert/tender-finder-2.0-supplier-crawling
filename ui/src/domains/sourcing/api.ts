...
export interface CrawlerConfig {
    ...
    // Company Crawler Settings
    tk_url?: string;
    db_url?: string;
    scrape_tk?: boolean;
    scrape_db?: boolean;

    // Portal Crawler Settings
    ad_url?: string;
    scrape_ad?: boolean;
...
export const api = {
    ...
    removeJob: async (jobId: string) => {
        const response = await axios.delete(`${API_URL}/crawler/queue/${jobId}`);
        return response.data;
    },
    ...
    stopCrawler: async (crawlerId: string) => {
        const response = await axios.post(`${API_URL}/crawler/stop/${crawlerId}`);
        return response.data;
    },
    purgeQueue: async () => {
        const response = await axios.delete(`${API_URL}/crawler/queue`);
        return response.data;
    },
    getLogs: async (lines: number = 100) => {
        const response = await axios.get<{ logs: string[], file?: string }>(`${API_URL}/crawler/logs`, { params: { lines } });
        return response.data;
    },
    updateScheduleConfig: async (config: any) => {
        const response = await axios.post(`${API_URL}/crawler/schedule`, config);
        return response.data;
    },
    ...
    deleteTender: async (tenderId: string) => {
        await axios.delete(`${API_URL}/crawler/tenders/${tenderId}`);
    },
