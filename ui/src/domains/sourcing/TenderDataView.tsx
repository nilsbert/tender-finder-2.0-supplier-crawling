...
/**
 * View for displaying and filtering tender data stored in the database.
 * 
 * Includes live search by term, website filtering, date filtering (crawled since, due until),
 * and sortable columns. Implements infinite scrolling and a preview overlay
 * on hover for efficient data browsing.
 * 
 * @param props - Component properties.
 */
const TenderDataView: React.FC<TenderDataViewProps> = ({ dbMode }) => {
    const [tenders, setTenders] = useState<Tender[]>([]);
    const [loading, setLoading] = useState(false);
    const [searchText, setSearchText] = useState('');
    const [selectedWebsite, setSelectedWebsite] = useState('');
    const [crawledSince, setCrawledSince] = useState(''); // Options: '', '24h', '7d', '30d'
    const [dueUntil, setDueUntil] = useState(''); // Options: '', '7d', '30d', '90d'
    ...
    // P3: Detail Drawer State
    const [selectedTender, setSelectedTender] = useState<Tender | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);

    // P3: Global Filter Management
    ...
    const fetchStats = useCallback(async () => {
        if (dbMode === 'disconnected') return;
        try {
            const data = await api.getTenderStats();
            setStats(data);
        } catch (e) {
    ...
    const fetchTenders = useCallback(async (reset = false) => {
        ...
        try {
            ...
            // Calculate crawled_since based on filter
            ...
            // Calculate due_until based on filter (future dates)
            ...
            if (reset) {
                ...
                if (data.length < limit) {
                    ...
                    setFilteredCount(-1); // -1 means "more available"
                }
            } else {
    ...
    }, [dbMode, searchText, selectedWebsite, page, limit, crawledSince, dueUntil, tenders]);
    ...
    useEffect(() => {
        ...
        }, 300); // 300ms debounce for search text

        return () => clearTimeout(timer);
    }, [searchText, selectedWebsite, crawledSince, dueUntil, dbMode, fetchTenders]);
    ...
    // Sort tenders based on current sort settings
        ...
        return (
            <div className="p-content-wrapper">
                <div style={{ textAlign: 'center', padding: '48px 0' }}>
                    <PHeading size="medium">Database Disconnected</PHeading>
                    <PText>Please connect to Cosmos DB in settings to view tender data.</PText>
                </div>
            </div>
        );
    }
            ...
            <PFlex direction="column" style={{ gap: '24px' }}>
                <PFlexItem>
                    <PFlex justifyContent="space-between" alignItems="center">
                        <PHeading size="large">Tender Data</PHeading>
                        {stats && (
                            <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                                <PTag color="notification-warning">
                                    Results: {filteredCount === -1 ? `${tenders.length}+` : filteredCount}
                                </PTag>
                                <PTag color="background-base">Total in DB: {stats.total}</PTag>
                            </div>
                        )}
                    </PFlex>
                </PFlexItem>

                {/* P3: Global Filter Header */}
                <PFlexItem>
                    <GlobalFilterHeader
