...
export const CrawlerQueueList: React.FC = () => {
    ...
    const sortedQueued = status && Array.isArray(status.queued) 
        ? [...status.queued].sort((a, b) => {
    ...
    const handleRemove = async (jobId: string) => {
        try {
            ...
            console.error(e);
            // setError("Failed to remove job"); // Don't block whole UI on error
        }
    };
    ...
    return (
        <div style={{ marginTop: '32px', marginBottom: '32px', padding: '24px', backgroundColor: 'white', border: '1px solid #e0e0e0', borderRadius: '4px' }}>
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            <PButton
                                ...
                                onClick={async () => {
                                    try {
                                        await api.restartWorkers();
                                        // Slight delay to allow workers to spin up before refresh
                                        setTimeout(fetchData, 1000);
                                    } catch (e) {
                                        console.error(e);
                                    }
                                }}
                            >Start Workers</PButton>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {/* Active Jobs */}
                {status && Array.isArray(status.active) && status.active.map((job) => (
                    <div key={`active-${job.job_id}`} style={{
                        padding: '16px',
                ...
                {sortedQueued.map((job, index) => (
                    <div key={`queued-${job.job_id}`} style={{
                        padding: '16px',
