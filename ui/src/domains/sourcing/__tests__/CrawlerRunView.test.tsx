...
describe('CrawlerRunView', () => {
    ...
    it('calls stop crawler API when stop button is clicked', async () => {
        vi.mocked(api.getCrawlerStatus).mockResolvedValue({ status: 'running' })
        vi.mocked(api.stopCrawler).mockResolvedValue({ message: 'Stopping' })
        ...
        const stopButtons = screen.getAllByLabelText('Stop')
        fireEvent.click(stopButtons[0]) // Click first stop button

        expect(window.confirm).toHaveBeenCalled()
        // Note: The actual crawlerId depends on which crawler is "running" in the mock.
        // Since we mock getCrawlerStatus to return 'running' for ALL calls in the loop,
        // the first one rendered (TED Austria) will be clicked.
        expect(api.stopCrawler).toHaveBeenCalledWith('ted_austria')
    })

    it('calls purge queue API when requested', async () => {
        vi.mocked(api.purgeQueue).mockResolvedValue({ message: 'Purged' })
        window.confirm = vi.fn(() => true)

        render(<CrawlerRunView dbMode="cosmos" />)

        const purgeButton = screen.getByText('Warteschlange leeren') // Using translation key or mock text
        fireEvent.click(purgeButton)

        expect(window.confirm).toHaveBeenCalled()
        expect(api.purgeQueue).toHaveBeenCalled()
    })
})
