...
from models.config import ConfigRetentionHistoryORM, ConfigRetentionORM
...
async def update_retention_config(payload: ConfigUpdate, db: AsyncSession = Depends(get_db)):
    ...
    history = ConfigRetentionHistoryORM(
        version=obj.version, change_summary=payload.change_summary, created_by=payload.created_by, **orm_to_dict(obj)
    )
    db.add(history)
...
async def get_retention_history(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(ConfigRetentionHistoryORM).order_by(ConfigRetentionHistoryORM.version.desc()).limit(10)
    )
